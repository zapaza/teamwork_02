...
const Game: React.FC = () => {
    ...
    const [planePosition, setPlanePosition] = useState<Position>({ x: 400, y: 550 }); // Для перерисовки самолета
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            switch (event.key) {
                case 'ArrowLeft':
                    planePositionRef.current.x = Math.max(planePositionRef.current.x - 10, 0);
                    break;
                case 'ArrowRight':
                    planePositionRef.current.x = Math.min(planePositionRef.current.x + 10, 1200);
                    break;
                case 'ArrowUp':
                    planePositionRef.current.y = Math.max(planePositionRef.current.y - 10, 0);
                    break;
                case 'ArrowDown':
                    planePositionRef.current.y = Math.min(planePositionRef.current.y + 10, 800);
                    break;
                case ' ':
                    setBullets(bullets => [...bullets, { id: Math.random(), ...planePositionRef.current }]); // Создаем новую пулю на позиции самолета
                    break;
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            if (Math.random() < 0.05) {
                ...
                setAsteroids(asteroids => [...asteroids, { id: Math.random(), x, y: 0 }]);
            }

            setBullets(bullets =>
                bullets.map(bulletPos => ({ ...bulletPos, y: bulletPos.y - 10 })).filter(bulletPos => bulletPos.y > 0)
            );
            ...
            setAsteroids(asteroids =>
                asteroids.map(pos => ({ ...pos, y: pos.y + (4 + score * 0.01) })) // увеличиваем скорость астероидов
                    .filter(pos => {
                        if (pos.y > 800) {
