...
export default class BoundaryManager {
   ...
   */
  static hitBoundaryConditional(character: IPacman | IGhost, boundary: IBoundary, { velocity }: {velocity: ICoordinates}) {
    const padding = boundary.width / 2 - character.radius - 1;
    return character.position.y - character.radius + velocity.y <=
