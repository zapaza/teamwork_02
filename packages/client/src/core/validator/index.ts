...
export const AuthSchema = yup
  .object({
    login: yup.string().required().min(3).max(20)
      .matches(/^[a-zA-Z0-9-_]*$/, { message: 'Can contain only latin symbol or numbers' })
      .test(
        'notNumbers',
    ...
    password: yup.string().required().min(8).max(40).test(
    ...
    first_name: yup.string().required()
      .matches(/^[a-zA-Zа-яА-Я-]*$/, { message: 'Can\'t contain space and special symbols' })
      .test(
